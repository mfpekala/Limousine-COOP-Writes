...
namespace pgm
{
    ...
    public:
        ...
        void build(RandomIt first, RandomIt last, size_t epsilon, size_t epsilon_recursive)
        {
            ...
            while (epsilon_recursive && last_n > 1)
            {
                ...
                {
                    if (rec_entries.size() > 0)
                    {
                        // TODO: Why is this zero sometimes?
    ...
    public:
        ...
        void internal_insert(size_t split_level, size_t low_split_ix, size_t high_split_ix, std::vector<Segment> &new_segs)
        {
            ...
            Entry representative = levels[split_level][low_split_ix].to_entry();
            if (split_level >= height() - 1)
            {
                ...
                for (auto s : new_segs)
                {
                    new_entries.push_back(Entry(s.data[0].first, s.data[0].second));
                }
                while (last_size > 1)
                {
                    EntryVector temporary_fill;
            ...
            else
            {
                ...
                // TODO: Make this cleaner using std stuff, was giving valgrind errors
                std::vector<Entry> new_entries;
                for (auto s : new_segs)
                {
                    new_entries.push_back(Entry(s.data[0].first, s.data[0].second));
                }
                EntryVector new_parent_data;
                size_t offset = std::distance(parent_seg->data.begin(), og_iter_into_parent);
                size_t window_size = high_split_ix - low_split_ix;
                for (size_t ix = 0; ix < offset; ++ix)
                {
                    new_parent_data.push_back(parent_seg->data[ix]);
                }
                for (auto el : new_entries)
                {
                    new_parent_data.push_back(el);
                }
                for (size_t ix = offset + window_size; ix < parent_seg->data.size(); ++ix)
                {
                    new_parent_data.push_back(parent_seg->data[ix]);
                }
                parent_seg->data = new_parent_data;
                if (can_absorb)
                {
                ...
                std::vector<Segment> new_parent_level;
                for (size_t ix = 0; ix < low_split_ix; ++ix)
                {
                    new_parent_level.push_back(levels[split_level][ix]);
                }
                for (auto s : new_segs)
                {
                    new_parent_level.push_back(s);
                }
                for (size_t ix = high_split_ix; ix < levels[split_level].size(); ++ix)
                {
                    new_parent_level.push_back(levels[split_level][ix]);
                }
                levels[split_level] = new_parent_level;
                if (!can_absorb)
                {
        ...
        {
            Entry og_rep = levels[split_level][splitting_ix].to_entry();
            size_t og_parent_ix = by_level_segment_ix_for_key(og_rep.first, split_level + 1);
            // First find the low ix
            size_t low_seg_ix = splitting_ix;
            size_t low_check_ix = low_seg_ix;
            Entry low_rep = levels[split_level][low_check_ix].to_entry();
            size_t low_parent_ix = by_level_segment_ix_for_key(low_rep.first, split_level + 1);
            while (low_check_ix > 0 && splitting_ix - low_seg_ix < split_neighborhood)
            {
                low_check_ix--;
                low_rep = levels[split_level][low_check_ix].to_entry();
                low_parent_ix = by_level_segment_ix_for_key(low_rep.first, split_level + 1);
            ...
            {
                Entry high_rep = levels[split_level][high_seg_ix].to_entry();
                size_t high_parent_ix = by_level_segment_ix_for_key(high_rep.first, split_level + 1);
                while (high_seg_ix < levels[split_level].size() && high_seg_ix - splitting_ix < split_neighborhood)
                {
                    ...
                    high_rep = levels[split_level][high_seg_ix].to_entry();
                    high_parent_ix = by_level_segment_ix_for_key(high_rep.first, split_level + 1);
        ...
        void split(size_t split_level, size_t splitting_ix)
        {
            ...
            {
                n += levels[split_level][seg_ix].data.size() + levels[split_level][seg_ix].buffer.size();
            }
            ...
            auto in_fun = [&](auto i)
            {
                if (proper_ix >= seg_iter->data.size() && buffer_ix >= seg_iter->buffer.size())
                {
                ...
                bool is_proper;
                if (proper_ix >= seg_iter->data.size())
                {
                    is_proper = false;
                }
...
#pragma pack(push, 1)
    ...
    struct BufferedPGMIndex<K, V>::Segment
    {
        ...
        Segment(const buffered_pgm_type *super, K first_x, float slope, int32_t intercept) : super(super),
                                                                                             first_x(first_x),
                                                                                             slope(slope),
                                                                                             intercept(intercept){};
        ...
        std::pair<std::vector<size_t>, std::vector<size_t>> get_padding() const
        {
            ...
            for (size_t ix = 0; ix < data.size(); ix++)
            {
                ...
                padding_top.push_back(ix + super->epsilon_value - approx_pos);
                padding_bottom.push_back(approx_pos - (ix - super->epsilon_value));
            }
