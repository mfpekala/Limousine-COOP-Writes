...
//protected:
    ...
    std::vector<V> keys;              ///< The sorted keys. 
    std::vector<V> values;              ///< The values associated with the keys. 
    // NOTE: For now, just trying to get the implementation off the ground so these values^ will be sorted once and that's it
    ...
    void build(RandomIt first, RandomIt last,
        ...
        // NOTE: Not the most efficient but works as first stab to get the values
        // as a simple sorted array
        keys.reserve(n);
        values.reserve(n);
...
public:
     ...
     */
    explicit BufferedPGMIndex(const std::vector<std::pair<K, V>> &data) : BufferedPGMIndex(data.begin(), data.end()) {}
    ...
    BufferedPGMIndex(RandomIt first, RandomIt last)
        : n(std::distance(first, last)),
          ...
          values(),
          levels_offsets() {
        build(first, last, Epsilon, EpsilonRecursive, segments, values, levels_offsets);
    }
    ...
    /**
     * Finds the value of an element with key equivalent to @p key.
     * @param key the value of the element to search for
     * @return the value of the element with key equivalent to @p key, or the maximum value of @p V if no such element is found
    */
    V find(const K&key) const {
        // NOTE: Should probably also return an iterator here
        ApproxPos range = search(key);
        // NOT EFFICIENT. SHOULD BE BINARY SEARCH.
        // For now just linearly search the range to the key
        for (auto i = range.lo; i < range.hi; i++) {
            if (keys[i] == key) {
                return values[i];
            }
        }
        std::cout << "Not found in not buffer" << std::endl;
        // If not found, try to find it in the segments buffer
        auto it = segment_for_key(key);
        // Print the buffer
        for (auto& p : it->buffer) {
            std::cout << p.first << " " << p.second << std::endl;
        }
        // Print the length of the buffer
        std::cout << "Buffer length: " << it->buffer.size() << std::endl;
    ...
    void insert(const K& key, const V& value) {
        ...
        // Print the length of the buffer
        std::cout << "Buffer length: " << seg.buffer.size() << std::endl;
    }
...
struct BufferedPGMIndex<K, V, Epsilon, EpsilonRecursive, Floating>::Segment {
    ...
    std::vector<std::pair<K, V>> buffer;  ///< A buffer of inserts waiting to come to this segment
